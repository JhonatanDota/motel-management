...
import ConsumableItemModel, {ConsumableItemAddModel} from "../models/ConsumableItemModel";

const CONSUMABLE_ITEMS_URL = `${API_BASE_URL}consumable-items/`;

export async function addConsumableItem(data: ConsumableItemAddModel): Promise<
AxiosResponse<ApiResponse<ConsumableItemModel>>
> {
  const response = await axios.post<ApiResponse<ConsumableItemModel>>(
    CONSUMABLE_ITEMS_URL, data
  );
